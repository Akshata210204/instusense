...
with st.spinner("🔄 Running Preprocessing..."):
    X_train, y_train, scaler, label_encoder, training_columns = load_and_preprocess(train_file, training=True)
...
with col1:
    st.markdown("<div style='background-color:#cce5ff;padding:15px;border-radius:10px;text-align:center'><b>Total Params</b><br><h3>104,195 (407 KB)</h3></div>", unsafe_allow_html=True)
with col2:
    st.markdown("<div style='background-color:#d4edda;padding:15px;border-radius:10px;text-align:center'><b>Trainable Params</b><br><h3>104,195 (407 KB)</h3></div>", unsafe_allow_html=True)
with col3:
    st.markdown("<div style='background-color:#f8d7da;padding:15px;border-radius:10px;text-align:center'><b>Non-Trainable Params</b><br><h3>0 (0.00 B)</h3></div>", unsafe_allow_html=True)
...
report_dict = classification_report(y_train, y_train_pred_classes, target_names=label_encoder.classes_, output_dict=True)
...
def load_test_assets():
    ...
    unnamed_cols = [col for col in df_test.columns if "unnamed" in col.lower() or "index" in col.lower()]
    if unnamed_cols:
        df_test = df_test.drop(columns=unnamed_cols)

    # Drop empty rows & difficulty column if present
...
# Load test dataset and assets
df_test, model, scaler, label_encoder, training_columns = load_test_assets()

# Row number input
row_num = st.number_input(
    "Enter row number from test dataset:",
    min_value=1,
    max_value=len(df_test),
    value=47,
    step=1
)

# --------------------------
# Prediction (from test.py)
# --------------------------
if st.button("Predict Row"):
    status = st.empty()
    status.info(f"🔍 Predicting row {row_num}...")

    # ✅ Select correct row (0-based)
